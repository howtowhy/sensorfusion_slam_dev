...
class KittiDataset(Dataset):
    def __init__(self, path, name, associations=None, type=DatasetType.KITTI): 
        ...
        self.fps = 10
        #self.timestamps = np.loadtxt(self.path + '/image_' + self.name + '/timestamps.txt')
        f = open(self.path + '/image_' + self.name + '/timestamps.txt', 'r')
        s = "."
        self.timestamps = []
        while s :
            s = f.readline()
            ...
            if (ss[0]!=''):
                sss = ss[1].split(":")
                #print(sss)
                self.timestamps.append(float(sss[0])*60*60 + float(sss[1])*60 + float(sss[2]))
    ...
    def getImage(self, frame_id):
        ...
        if frame_id < self.max_frame_id:
            try:
                #print("here i am !!!")
                #print(self.path + '/image_' + self.name + '/data/' + str(frame_id).zfill(10) + '.png')
                img = cv2.imread(self.path + '/image_' + self.name + '/data/' + str(frame_id).zfill(10) + '.png')
